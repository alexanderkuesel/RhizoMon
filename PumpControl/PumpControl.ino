//Retrofit of Boiler Control program to do Pump Control instead
...
const char CASE_TEMP_topic[]  = "MUTHUR/PND/NDATA/PMP/Case_Temp";
const char FLOW_RATE_topic[] = "MUTHUR/PND/NDATA/PMP/Flow_Rate";
const char SW_STATE_topic[] = "MUTHUR/PND/NDATA/PMP/Master_Switch";
const char RELAY_READ_topic[] = "MUTHUR/PND/NDATA/PMP/Relay_State";
const char sub_topic[] = "testtopic";
const char HEART_BEAT_topic[] = "MUTHUR/PND";
// ------------------*****---------------------
...
int FLOW_RATE = 2;
int SW_STATE = 5; // switch state input
...
void setup()
{
  ...
  //!FlowRate
  waterFlow = 0;
  attachInterrupt(digitalPinToInterrupt(FLOW_RATE), pulse, RISING);
    //sei();
  currentTime = millis();
...
void loop()
{
 ...
 // Every second, calculate and print litres/hour
  ...
  // Pulse frequency (Hz) = 7.5Q, Q is flow rate in L/min.
    l_hour = (waterFlow * 60 / 7.5);
    l_min = (waterFlow / 7.5);
  // (Pulse frequency x 60 min) / 7.5Q = flowrate in L/hour

    waterFlow = 0; // Reset Counter
  ...
  display.showNumberDec(waterFlow); //uses int
